...
from vyper.ast.nodes import Break, Pass, Continue, Log, Raise, Return, AugAssign, Assign, Assert, AnnAssign, Index
...
def get_node(node_class, *args, **kwargs):
    ...
    if len(args) != 0:
        ...
        for slot in node_class.__slots__[len(args):]:
            args_dict[slot] = None
        args = []
    new_node = node_class(ast_type=node_class.__name__, node_id=next_nodeid(), *args, **args_dict)
